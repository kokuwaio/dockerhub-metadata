...
RUN ARCH=$(uname -m) && \
	...
	[[ -z ${SUFFIX:-} ]] && echo "Unknown arch: $ARCH" && exit 1; \
	wget -q "https://github.com/stunnel/static-curl/releases/download/8.21.0/curl-linux-$SUFFIX-glibc-8.21.0.tar.xz" --output-document=- | tar --xz --extract -o --directory=/usr/local/bin curl && \
