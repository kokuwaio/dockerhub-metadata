...
RUN ARCH=$(uname -m) && \
	...
	wget -q "https://github.com/jqlang/jq/releases/download/jq-1.8.2/jq-linux-$SUFFIX" --output-document=/usr/local/bin/jq && \
...
RUN ARCH=$(uname -m) && \
	...
	[[ -z ${SUFFIX:-} ]] && echo "Unknown arch: $ARCH" && exit 1; \
	wget -q "https://github.com/stunnel/static-curl/releases/download/8.20.0/curl-linux-$SUFFIX-glibc-8.20.0.tar.xz" --output-document=- | tar --xz --extract -o --directory=/usr/local/bin curl && \
